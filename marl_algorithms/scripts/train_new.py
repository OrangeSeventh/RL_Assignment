...
def train_algorithm(algorithm_name, num_iterations=1000):
    ...
    ppo_config_obj = ppo_config_obj.environment(
        VMASEnv,
        env_config=ENV_CONFIG,
        disable_env_checking=True,
    )
    ppo_config_obj = ppo_config_obj.framework("torch")

    # 添加训练参数
    ppo_config_obj = ppo_config_obj.training(
    ...
    # 添加多智能体配置
    multiagent_config = algo_config.get("multiagent", {})
    for key, value in multiagent_config.items():
        ppo_config_obj.multiagent[key] = value

    # 添加评估配置
    ppo_config_obj.evaluation(
